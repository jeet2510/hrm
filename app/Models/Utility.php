...
class Utility extends Model
{
    ...
    public static function employeePayslipDetail($employeeId, $month)
    {
        ...
        $taxDeduction      = PaySlip::where('employee_id', $employeeId)->where('salary_month', $month)->get('tax_deduction');

        $decodedTaxDeduction = json_decode($taxDeduction[0]->tax_deduction, true);
        $payslip['taxDeduction'] = $decodedTaxDeduction;
        // dd($decodedTaxDeduction);
        $allTaxDeductionData = $decodedTaxDeduction['deduction_details'] ?? [];
        // dd($allTaxDeductionData);

        // $employess = Employee::find($employeeId);
        $tax_deduction_amount = $decodedTaxDeduction['final_tax'] ?? 0;
        // dd($tax_deduction_amount);
        // $arrayJson = json_decode($taxDeduction);
        // dd($arrayJson );
        // foreach ($decodedTaxDeduction as $taxDeduction) {
        //     $deduc = json_decode($taxDeduction->tax_deduction);
        //     dd($deduc);
        //     // $title = $deduc->title ?? '-';
        //     $total_tax = $deduc->tax_grand_total ?? 0;
        //     $total_deduct_from_tax = $deduc->tax_to_be_minus_from_grand_tax ?? 0;
        //     $tax_deduction_amount = ($total_tax - $total_deduct_from_tax) ?? 0;
        //     // $tax_deduction_amount = (($deduc->tax_grand_total ?? 0) - ($deduc->amount_deduction_from_tax ?? 0)) ?? 0;
...
<<<<<<< HEAD
        ...
        if($advance_payment_detail->advance_payment_deduction != "null"){
            $advance_details = json_decode($advance_payment_detail->advance_payment_deduction);
            $approved_amount = $advance_details->approved_amount;
            // dd($advance_details->approved_amount);
        } else {
...
>>>>>>> 200d3e83 (Initial commit)
    ...
    public static function advaceSalaryInfo($employeeId, $month){
        // $month = '2024-04';
        $info = AdvanceSalary::where('employee_id',$employeeId)->where('advance_salary_month', $month)->get();
        // dd($info);
    }
