                                ...
                                <table class="table table-striped table-hover table-md">
                                    <tbody>
                                        ...
                                        @foreach ($payslipDetail['deduction']['deduction'] as $deduction)
                                            ...
                                            @endforeach
                                        @endforeach
                                        {{-- {{ dd($payslipDetail['taxDeduction']['total_tax']) }} --}}
                                        @if ($payslipDetail['taxDeduction']['final_tax'] > 0)
                                            <tr>
                                                <td>{{ __('Tax Deduction') }}</td>
                                                <td>{{ $payslipDetail['taxDeduction']['tax_title'] }}</td>
                                                {{-- Access 'title' key from the first element of the array --}}
                                                <td> - </td>
                                                <td class="text-right">
                                                    {{ \Auth::user()->priceFormat($payslipDetail['taxDeduction']['final_tax'] ?? 0) }}
